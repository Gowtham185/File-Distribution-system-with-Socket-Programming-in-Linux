...
//port and directory for stext server
#define PORT 32323
...
// Function to check if the directory exists if not create
...
void handle_upload(int client_sock, const char *filename, const char *destination_path) {
 char buffer[BUFFER_SIZE];
    ...
    // checking if the directory exists, including any intermediate folders
    ensure_directory_exists(destination_path);

    // saving file locally in Stext
...
void handle_tar(int client_sock, const char *file_type) {
    ...
    // Create the tar file in the ~/stext directory
    snprintf(tar_file_path, sizeof(tar_file_path), "%s/%s", STEXT_DIR, tar_filename);
    snprintf(buffer, sizeof(buffer), "tar -cvf %s %s", tar_file_path, STEXT_DIR);
    system(buffer);
...
void handle_display(int client_sock, const char *pathname) {
    ...
    // List .txt files in the specified directory in Stext
    snprintf(command, sizeof(command), "find %s -type f -name '*.txt'", pathname);
    FILE *fp = popen(command, "r");
    if (fp == NULL) error("ERROR listing .txt files");
